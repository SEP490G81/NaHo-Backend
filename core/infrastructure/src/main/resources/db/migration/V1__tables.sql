...
ALTER TABLE payment_idempotencies
    ADD CONSTRAINT fk_payment_idempotency_order FOREIGN KEY (payment_order_id) REFERENCES payment_orders (id);

ALTER TABLE payment_idempotencies
    ADD CONSTRAINT fk_payment_idempotency_user FOREIGN KEY (user_id) REFERENCES users (id);

CREATE UNIQUE INDEX uk_payment_orders_pending_user ON payment_orders (pending_user_id);
CREATE INDEX idx_payment_orders_user_status ON payment_orders (user_id, status);
CREATE INDEX idx_payment_idempotency_order ON payment_idempotencies (payment_order_id);