...
class AnalyzeSpeakingTest {
        ...
        void UTCID01_Should_AnalyzeSuccessfully_When_ValidRequestWithTopic() {
                ...
                when(answerHistoryRepositoryPort.saveSpeechAssessment(any(SpeechAssessment.class)))
                                .thenReturn(savedSpeechAssessment);
                when(aiAnalysisPort.analyzeSpeaking(any(AiAnalysisPort.Context.class))).thenReturn(rawLlmFeedback);
                ...
                verify(fileStorageInputPort, times(1))
                                .uploadFile(argThat(cmd -> cmd.getFolderName().equals("recordings") &&
                                                cmd.getOriginalName().equals("test.wav") &&
                                                cmd.getContentType().equals("audio/wav") &&
                                                cmd.getSize() == 3));
                verify(answerHistoryRepositoryPort, times(1))
                                .saveAnswerHistory(argThat(history -> history.getUserId().equals(1L) &&
                ...
                verify(answerHistoryRepositoryPort, times(1)).saveSpeechAssessment(
                                argThat(assessment -> assessment.getAnswerHistoryId().equals(100L) &&
                ...
                verify(aiAnalysisPort, times(1)).analyzeSpeaking(any(AiAnalysisPort.Context.class));
                verify(answerHistoryRepositoryPort, times(1))
                                .saveContentAssessment(argThat(content -> content.getAnswerHistoryId().equals(100L) &&
        ...
        void UTCID03_Should_AnalyzeSuccessfully_When_LlmFeedbackCorrupted() {
                ...
                when(answerHistoryRepositoryPort.saveSpeechAssessment(any(SpeechAssessment.class)))
                                .thenReturn(savedSpeechAssessment);
                when(aiAnalysisPort.analyzeSpeaking(any(AiAnalysisPort.Context.class))).thenReturn(corruptedFeedback);
                ...
                assertNotNull(result);
                assertEquals(100L, result.historyId());
                // Since pronunciation score is 80.0 (pronScore10 = 8.0) and other scores fail
                // back to 0.0
